...
interface PatientLabOrderModalProps {
  isOpen: boolean;
  onClose: () => void;
  order?: PatientLabOrder | null;
  patient: PatientDetail;
}

export function PatientLabOrderModal({
  isOpen,
  onClose,
  order,
  patient,
}: PatientLabOrderModalProps) {
  ...
  const onSubmit = async (data: CreateLabOrderFormData) => {
    try {
      const labOrderData = {
        ...
        affiliate: patient.affiliates?.[0],
        lab: data.lab,
      ...
      await createPatientLabOrder(labOrderData).unwrap();
